...
import copy
import time
import MOCAP.DataDescriptions as DataDescriptions
...
#Unpack Mocap Data Functions
    ...
    def extractdatas(self, texte):
        listedata = texte.split("\n")
        nbdataset = int(listedata[0].split(':')[1])
        encours = 1
        for _ in range(nbdataset):
            while listedata[encours][:9] != "  Dataset":
                encours += 1
            while listedata[encours][:7] != "  Type:":
                encours += 1                
            if listedata[encours][:20] != "  Type: 1 Rigid Body":
                continue
            encours += 1
            while listedata[encours][:23] != "    Rigid Body Name   :":
                encours += 1
            nomrb = listedata[encours][24:]
            encours += 1
            while listedata[encours][:23] != "    ID                :":
                encours += 1
            self.rigidbodyinfo[int(listedata[encours][24:])] = nomrb
        return True
    ...
    def __process_message( self, data : bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA :
            ...
            self.extractdatas(data_descs_str)
            if print_level>0:
